...
class GameLogic:
    ...
    def _blend_colors(self, ball1: Ball, ball2: Ball):
        ...
        Использует улучшенную формулу смешивания для более реалистичного результата.
        Формула учитывает, что RGB - аддитивная модель (свет), поэтому смешивание должно сохранять яркость.
        ...
        # Используем формулу смешивания, которая лучше сохраняет яркость
        # Вместо простого усреднения используем формулу, учитывающую яркость компонентов
        # Это предотвращает затемнение при смешивании ярких цветов
        
        # Нормализуем значения в диапазон [0, 1] для вычислений
        r1_norm = r1 / 255.0
        g1_norm = g1 / 255.0
        b1_norm = b1 / 255.0
        r2_norm = r2 / 255.0
        ...
        # Используем формулу смешивания с учётом яркости
        # Для RGB (аддитивная модель) смешивание должно давать более яркий результат
        # Используем формулу: sqrt((color1^2 + color2^2) / 2) для сохранения яркости
        new_r_norm = math.sqrt((r1_norm * r1_norm + r2_norm * r2_norm) / 2.0)
        new_g_norm = math.sqrt((g1_norm * g1_norm + g2_norm * g2_norm) / 2.0)
        new_b_norm = math.sqrt((b1_norm * b1_norm + b2_norm * b2_norm) / 2.0)
        
        # Конвертируем обратно в диапазон [0, 255]
